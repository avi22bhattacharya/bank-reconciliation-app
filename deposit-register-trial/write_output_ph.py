"""
Generate PH Davie Bank Reconciliation Output Excel
...
JSON_IN = BASE + "recon_ph_results.json"
OUTPUT  = BASE + "PH_Davie_Bank_Recon_Output.xlsx"

# ── PH Davie constants ────────────────────────────────────────────────────────
ACCOUNT_INFO   = "WFB Account number 4573468451"
GL_ACCOUNT_ID  = "11100113 PH Davie - Ops - WFB   (Bank Account)"
PROP_LABEL_BANK = "Palm Haven^East Pine Ridge"
PROP_DISPLAY   = "Property: East Pine Ridge(114epr), Palm Haven (113phv)"
...
def period_range_str(all_gl):
    """Returns 'MM/DD/YY - MM/DD/YY  (accrual basis)' from GL date range."""
    dates = [parse_date2(g.get("date")) for g in all_gl if g.get("date")]
    ...
    import calendar
    first = datetime(mx.year, mx.month, 1)
    last  = datetime(mx.year, mx.month, calendar.monthrange(mx.year, mx.month)[1])
    return f"{first.strftime('%m/%d/%y')} - {last.strftime('%m/%d/%y')}  (accrual basis)"
...
def group_unrec_gl(unrec_items):
    ...
    Returns list of dicts: {date, type, key, desc, amount, items}
    """
    groups = OrderedDict()
    for g in sorted(unrec_items, key=lambda x: (parse_date2(x.get("date")) or datetime.min, x.get("control", ""))):
    ...
    for gkey, gdata in groups.items():
        ...
        if gkey[0] == "K":
            ...
            # J/R entries
            if is_debit:
                # AR: use Remarks for E column
                e_desc = first.get("remarks", "") or first.get("desc", "") or ""
            else:
                # AP journal: use Remarks for E column
                e_desc = first.get("remarks", "") or first.get("desc", "") or ""
...
# Sheet 1 – Summary
# ═══════════════════════════════════════════════════════════════════════════════
def build_summary(wb, r):
    ws = wb.create_sheet("Summary", 0)
    ws.sheet_view.showGridLines = False

    # Column widths to approximate reference
    ...
    # ── Header ────────────────────────────────────────────────────────────────
    wc(2, 2, "Bank Reconciliation - PH Davie", bold=True, size=14)
    wc(3, 2, ACCOUNT_INFO)
    ...
    # Bank ending balance: for WFB daily-sweep accounts this is 0
    # Computed: adjusted_bank + outstanding_bank_total (where adjusted = adjusted_book)
    ...
    # Bank deposit data rows (rows 11+)
    dep_data_start = 11
    ...
    wc(gl_hdr_row, 2, f"General Ledger Information - {GL_ACCOUNT_ID.split()[0]} PH Davie", bold=True)
...
def build_gl(wb, r):
    ...
    ws.cell(row=2, column=2, value=PROP_DISPLAY)
    ws.cell(row=3, column=2, value=f"Detail Date Range: {date_range_str}")
    ws.cell(row=4, column=1, value=GL_ACCOUNT_ID)
    gl_opening = r.get("gl_opening_balance", 0.0) or 0.0
...
# ═══════════════════════════════════════════════════════════════════════════════
def build_unrec_transactions(wb, r):
    ...
    for b in sorted(unrec_bank, key=lambda x: parse_date2(x.get("date")) or datetime.min):
        ...
        ws.cell(row=row, column=1, value=PROP_LABEL_BANK)
        ws.cell(row=row, column=2, value=b.get("date") or "")
    ...
    # AR subtotal
    ar_total_row = row
    write_total(ar_total_row, ar_total)
    row += 1
    ...
    # AP subtotal
    ap_total_row = row
    write_total(ap_total_row, ap_total)
...
def main():
    ...
    build_summary(wb, r)
    build_bank_statement(wb, r)
    build_gl(wb, r)
    build_unrec_transactions(wb, r)
